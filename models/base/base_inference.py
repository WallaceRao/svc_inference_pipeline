...
from utils.util import load_config, find_checkpoint_of_mapper
...
class BaseInference(object):
    def __init__(self, cfg, args):
        ...
        if (
            self.args.checkpoint_dir_of_vocoder is not None
            or self.args.checkpoint_file_of_vocoder is not None
        ):
    ...
    def get_vocoder_info(self):
        self.checkpoint_file_of_vocoder = self.args.checkpoint_file_of_vocoder
        if self.checkpoint_file_of_vocoder is None:
            assert self.args.checkpoint_dir_of_vocoder is not None
            #     if old_mode:
            #         max_steps = -1
            #         # Load max steps
            #         for file in glob.glob(
            #             os.path.join(self.args.checkpoint_dir_of_vocoder, "*.pt")
            #         ):
            #             epoch = int(file.split("/")[-1].split(".")[0])
            #             if epoch >= max_steps:
            #                 max_steps = epoch
            #                 self.checkpoint_file_of_vocoder = file
            #     else:
            #         checkpoint_path = os.path.join(
            #             self.args.checkpoint_dir_of_vocoder, "checkpoint"
            #         )
            #         checkpoint_filename = open(checkpoint_path).readlines()[-1].strip()
            #         self.checkpoint_file_of_vocoder = os.path.join(
            #             self.args.checkpoint_dir_of_vocoder, checkpoint_filename
            #         )
            vocoder_dir = os.path.abspath(self.args.checkpoint_dir_of_vocoder)
            ckpt_list = [ckpt for ckpt in Path(vocoder_dir).glob("*.pt")]
            ckpt_list.sort(key=lambda x: int(x.stem), reverse=True)
            self.checkpoint_file_of_vocoder = str(ckpt_list[0])
        else:
            vocoder_dir = os.path.dirname(self.checkpoint_file_of_vocoder)
        # self.vocoder_cfg = load_config(
        #     os.path.join(
        #         os.path.dirname(self.checkpoint_file_of_vocoder), "args.json"
        #     ),
        #     lowercase=True
        # )
        ...
        self.cfg.vocoder.model.bigvgan = self.cfg.vocoder.vocoder
        # self.vocoder_cfg = load_config(os.path.join(vocoder_dir, "args.json"), lowercase=True)
        # self.vocoder_cfg.model.bigvgan = self.vocoder_cfg.vocoder
        self.cfg.vocoder.preprocess = self.cfg.preprocess
        self.cfg.vocoder.preprocess.hop_length = self.cfg.vocoder.preprocess.hop_size
        self.vocoder_tag = self.checkpoint_file_of_vocoder.split("/")[-2].split(":")[-1]
        self.vocoder_steps = self.checkpoint_file_of_vocoder.split("/")[-1].split(".")[
            0
        ]

    def load_vocoder(self, checkpoint):
        if self.args.vocoder_name == "bigvgan":
            self.vocoder.load_state_dict(checkpoint["generator_state_dict"])
        else:
    ...
    def build_testdata_loader(self, args, target_singer):
        datasets, collate = self.build_test_dataset()
        self.test_dataset = datasets(self.cfg, args, target_singer)
    ...
    def inference_for_batches(self, args, target_singer):
        ###### Construct test_batch ######
        loader = self.build_testdata_loader(args, target_singer)
...
def base_parser():
    ...
    parser.add_argument(
        "--checkpoint_dir_of_vocoder",
        help="Vocoder's checkpoint dir including model file and configuration",
        type=str,
        default=None,
    )
    parser.add_argument(
        "--checkpoint_file_of_vocoder",
        help="Vocoder's checkpoint file",
        type=str,
        default=None,
    )
    parser.add_argument(
