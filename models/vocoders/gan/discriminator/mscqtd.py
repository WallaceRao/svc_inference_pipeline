...
class DiscriminatorCQT(nn.Module):
    def __init__(self, cfg, hop_length, n_octaves, bins_per_octave):
        ...
        self.filters = cfg.model.mscqtd.filters
        self.max_filters = cfg.model.mscqtd.max_filters
        self.filters_scale = cfg.model.mscqtd.filters_scale
        self.kernel_size = (3, 9)
        self.dilations = cfg.model.mscqtd.dilations
        self.stride = (1, 2)

        self.in_channels = cfg.model.mscqtd.in_channels
        self.out_channels = cfg.model.mscqtd.out_channels
...
class MultiScaleCQTDiscriminator(nn.Module):
    def __init__(self, cfg):
        super(MultiScaleCQTDiscriminator, self).__init__()
        ...
        self.discriminators = nn.ModuleList(
            [
                DiscriminatorCQT(
                    cfg,
                    hop_length=cfg.model.mscqtd.hop_lengths[i],
                    n_octaves=cfg.model.mscqtd.n_octaves[i],
                    bins_per_octave=cfg.model.mscqtd.bins_per_octaves[i],
                )
                for i in range(len(cfg.model.mscqtd.hop_lengths))
            ]
        )
