        ...
        if (
            ...
            self.activations = nn.ModuleList(
                [
                    Activation1d(
                        activation=Snake(
                            channels, alpha_logscale=cfg.snake_logscale
                        )
                    )
...
class Generator(torch.nn.Module):
    ...
    def __init__(self, cfg):
        ...
        resblock = AMPBlock1 if cfg.resblock == "1" else AMPBlock2
        ...
        for i in range(len(self.ups)):
            ...
            for j, (k, d) in enumerate(
                ...
                self.resblocks.append(
                    resblock(cfg, ch, k, d, activation=cfg.activation)
                )

        # post conv
        if (
            cfg.activation == "snake"
        ):  # periodic nonlinearity with snake function and anti-aliasing
            activation_post = Snake(ch, alpha_logscale=cfg.snake_logscale)
            self.activation_post = Activation1d(activation=activation_post)
        elif (
            cfg.activation == "snakebeta"
        ):  # periodic nonlinearity with snakebeta function and anti-aliasing
            activation_post = SnakeBeta(ch, alpha_logscale=cfg.snake_logscale)
            self.activation_post = Activation1d(activation=activation_post)
        else:
            raise NotImplementedError(
                "activation incorrectly specified. check the config file and look for 'activation'."
...
class DiscriminatorR(nn.Module):
    def __init__(self, cfg, resolution):
        ...
        norm_f = (
            weight_norm if cfg.use_spectral_norm == False else spectral_norm
        )
        if cfg.mrd_override:
            print(
                "INFO: overriding MRD use_spectral_norm as {}".format(
                    cfg.VOCODER.MRD_USE_SPECTRAL_NORM
                )
            )
            norm_f = (
                weight_norm
                if cfg.VOCODER.MRD_USE_SPECTRAL_NORM == False
                else spectral_norm
            )
        self.d_mult = cfg.discriminator_channel_mult
        if cfg.mrd_override:
            print(
                "INFO: overriding mrd channel multiplier as {}".format(
                    cfg.VOCODER.MRD_CHANNEL_MULT
                )
            )
            self.d_mult = cfg.VOCODER.MRD_CHANNEL_MULT
...
#         self.mpd = MultiPeriodDiscriminator(self.cfg)
#         self.mrd = MultiResolutionDiscriminator(self.cfg)

#     def _forward_gen(self, x):
#         y_pred = self.generator(x)
#         return y_pred

#     def _forward_mpdisc(self, y, y_pred):
#         y_mpd_r, y_mpd_g, f_mpd_r, f_mpd_g = self.mpd(y, y_pred)

#         return y_mpd_r, y_mpd_g, f_mpd_r, f_mpd_g

#     def _forward_mrdisc(self, y, y_pred):
#         y_mrd_r, y_mrd_g, f_mrd_r, f_mrd_g = self.mrd(y, y_pred)

#         return y_mrd_r, y_mrd_g, f_mrd_r, f_mrd_g
