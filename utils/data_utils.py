...
def pitch_shift_to_target(frame_pitch, target_pitch_median, source_pitch_median=None):
    ...
    print("Auto transposing...")

    factor = get_conversion_f0_factor(
...
def load_phone_pitch(
    ...
    if utt2spk is None:
        ...
        for utt_info in meta_data:
            utt = utt_info["Dataset"] + "_" + utt_info["Uid"]
            pitch_path = os.path.join(pitch_dir, f"{utt}.npy")
            ...
            pitch_scaler.partial_fit(phone_pitch.reshape(-1, 1))
...
# TODO: interoperate & sentence-level norm
def load_phone_energy(
    meta_data,
    processed_dir,
    energy_dir,
    utt2dur,
    ...
    if utt2spk is None:
        ...
        for utt_info in meta_data:
            ...
            frame_energy = np.load(energy_path)
            assert len(frame_energy) > 0
            phone_energy = phone_average_pitch(frame_energy, utt2dur[utt], interoperate)
            if use_log_scale:
                nonzero_idxes = np.where(phone_energy != 0)[0]
                phone_energy[nonzero_idxes] = np.log(phone_energy[nonzero_idxes])
            utt2energy[utt] = phone_energy
            energy_scaler.partial_fit(phone_energy.reshape(-1, 1))
        ...
        for spk in spk2utt:
            ...
            for utt in spk2utt[spk]:
                ...
                frame_energy = np.load(energy_path)
                assert len(frame_energy) > 0
                phone_energy = phone_average_pitch(
                    frame_energy, utt2dur[utt], interoperate
                )
                if use_log_scale:
                    nonzero_idxes = np.where(phone_energy != 0)[0]
                    phone_energy[nonzero_idxes] = np.log(phone_energy[nonzero_idxes])
