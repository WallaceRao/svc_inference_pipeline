...
def get_f0_features_using_pyin(audio, fs, win_length, hop_length, f0_min, f0_max):
...
def get_f0_features_using_parselmouth(
    ...
    """
    hop_size = int(np.round(cfg.hop_length * speed))

    # Calculate the time step for pitch extraction
    time_step = hop_size / cfg.fs * 1000

    f0 = (
        parselmouth.Sound(audio, cfg.fs)
        .to_pitch_ac(
    ...
    # Get the coarse part
    pitch_coarse = f0_to_coarse(f0, cfg.pitch_bin, cfg.pitch_min, cfg.pitch_max)
    return f0, pitch_coarse


def get_f0_features_using_dio(audio, mel_len, fs, hop_length, f0_min, f0_max):
    ...
    # Get the f0
    f0 = pw.stonemask(audio.astype("double"), _f0, t, fs)
